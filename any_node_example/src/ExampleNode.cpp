...
void ExampleNode::init()
{
  ...
  const double worker_timestep = 2.0;
...
bool ExampleNode::myWorkerFunction(const any_worker::WorkerEvent& event)
{
  // called by the worker created in init() function
  MELO_INFO("myWorkerFunction called");
  return true;
}
